    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 24) {
                ...
                if let error {
                    Text(error)
                        .foregroundColor(ThemeManager.shared.error)
                        .font(.caption)
                }
    ...
    private func submit() {
        ...
        Task {
            do {
                ...
                if isLogin {
                    resp = try await api.login(userId: userId, password: password, deviceId: deviceId)
                } else {
                    resp = try await api.register(userId: userId, name: name, password: password, deviceId: deviceId)
                }
