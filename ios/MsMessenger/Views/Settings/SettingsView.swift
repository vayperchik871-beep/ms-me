...
struct SettingsView: View {
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section("Внешний вид") {
                    Toggle("Тёмная тема", isOn: $theme.isDark)
                }

                if let user, user.isAdmin == true {
                    Section("Администрирование") {
                        Button(action: { showAdmin = true }) {
                            Label("Админ-панель", systemImage: "shield.fill")
                ...
                Section("О приложении") {
                    HStack {
                        Text("Версия")
                        Spacer()
                        Text("1.0.2").foregroundColor(theme.textSecondary)
                    }
                }
            }
