      ...
      <div className="app desktop-app" style={{ '--user-accent': accentColor }}>
        <div className="desktop-sidebar">
          ...
          <nav className="dsb-nav">
            {[
              { id: 'chats', icon: SidebarChatIcon },
              { id: 'contacts', icon: SidebarContactsIcon },
              { id: 'music', icon: SidebarMusicIcon },
              { id: 'settings', icon: SidebarSettingsIcon },
            ].map(({ id, icon: Icon }) => (
              <button
                key={id}
                className={`dsb-btn ${tab === id ? 'dsb-btn-active' : ''}`}
                onClick={() => setTab(id)}
                title={id === 'chats' ? 'Чаты' : id === 'contacts' ? 'Контакты' : id === 'music' ? 'Музыка' : 'Настройки'}
              >
                <Icon active={tab === id} />
              </button>
            ))}
          ...
          <div className="dsb-bottom">
            <button className="dsb-btn" onClick={() => setShowOnboarding(true)} title="Аккаунты">
              <SidebarAccountsIcon />
            </button>
          </div>
        </div>
...
function SidebarChatIcon({ active }) {
  return (
...
function SidebarContactsIcon({ active }) {
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill={active ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="1.8">
...
function SidebarMusicIcon({ active }) {
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill={active ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="1.8">
...
function SidebarSettingsIcon({ active }) {
  return (
